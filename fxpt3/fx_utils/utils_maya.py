...
def getNodeRelatives(node, parent=False, children=False, shapes=False, typ=None):
    if m.objExists(node):
        if typ:
            ...
            return m.listRelatives(
                node, fullPath=True, parent=parent, children=children, shapes=shapes
            )
...
def parentAPI(source, target, absolute=True):
    ...
    if absolute:
        ...
        worldMatrixPlug = srcFnDependencyNode.findPlug(
            'worldMatrix', False
    ...
    if absolute:
        ...
        srcFnTransform.setTranslation(
            srcWMtx.translation(om.MSpace.kWorld), om.MSpace.kWorld
        )
        srcFnTransform.setRotation(
            srcWMtx.rotation(asQuaternion=True), om.MSpace.kWorld
        )
...
def opFlagEraser():
    ...
    print(f'{counter} opposite flag(s) erased.')
