...
from fxpt3.fx_prefsaver.pstypes import UIType

from fxpt3.fx_prefsaver import ctrl_var
from fxpt3.fx_prefsaver import ctrl_qt

try:
    from fxpt3.fx_prefsaver import ctrl_maya
except ImportError:
    ctrl_maya = None

try:
    from fxpt3.fx_prefsaver import ctrl_pymel
except ImportError:
    ctrl_pymel = None
...
class PrefSaver(object):
    ...
    def addControl(self, control, uiType, defaultValue=None):
        ...
        if UIType.isTypeOf(uiType, UIType.TypesPYQT | UIType.TypesPYSIDE):
            controller = ctrl_qt.getController(uiType, control, defaultValue)
            if controller:
        ...
        if UIType.isTypeOf(uiType, UIType.TypesM):
            if ctrl_maya:
                self.controllers.append(ctrl_maya.getController(uiType, control, defaultValue))
            else:
                message('Failed to add controller (type={0}) for {1}'.format(uiType, str(control)))
            return
        ...
        if UIType.isTypeOf(uiType, UIType.TypesPM):
            if ctrl_pymel:
                self.controllers.append(ctrl_pymel.getController(uiType, control, defaultValue))
            else:
                message('Failed to add controller (type={0}) for {1}'.format(uiType, str(control)))
            return
    ...
    def addVariable(self, name, getter, setter, defaultValue):
        self.controllers.append(ctrl_var.CtrlVar(name, getter, setter, defaultValue))
